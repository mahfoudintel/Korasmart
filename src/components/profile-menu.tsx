...
import { Camera, ChevronDown, LogIn, LogOut, Save, UserRound } from "lucide-react";
...
export function ProfileMenu() {
  ...
  const { profile, updateProfile, login, logout } = useLocalProfile();
        ...
        <div className="absolute right-0 z-50 mt-3 w-[min(24rem,calc(100vw-2rem))] rounded-[20px] border border-white/10 bg-[#10190f]/95 p-4 shadow-2xl backdrop-blur-xl">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs leading-relaxed text-white/58">Local login for this prototype. Later this becomes your real account.</p>
          ...
          <div className="mt-4 grid gap-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[.05] p-3">
              ...
              <p className="mt-1 text-xs leading-relaxed text-white/45">Assigned to this login. Only your avatar and display name can be changed here.</p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {profile.loggedIn ? (
              <button onClick={logout} className="inline-flex h-11 items-center justify-center gap-2 rounded-full border border-orange-300/35 px-4 text-sm font-black text-orange-200">
                <LogOut className="h-4 w-4" />
                Logout
              </button>
            ) : (
              <button onClick={login} className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-lime-300 px-4 text-sm font-black text-black">
          ...
          <p className="mt-4 flex items-start gap-2 rounded-2xl bg-white/[.05] p-3 text-xs leading-relaxed text-white/58">
