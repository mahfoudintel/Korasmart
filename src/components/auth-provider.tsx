...
function LoginForm() {
  ...
  return (
    <main className="field-bg grid min-h-screen place-items-center px-4 py-10">
      <form onSubmit={login} className="w-full max-w-md rounded-[28px] border border-white/12 bg-black/35 p-6 shadow-2xl backdrop-blur-xl">
        <div className="grid h-14 w-14 place-items-center rounded-full bg-lime-300 text-black">
          <ShieldCheck className="h-8 w-8" />
        </div>
        <h1 className="mt-5 text-3xl font-black text-white">KoraSmart Login</h1>
        <p className="mt-2 text-sm leading-6 text-white/68">Use your group name and the temporary password shared privately.</p>
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      const { data } = await supabase
        .from("players")
        .select("id, name, username, must_change_password")
