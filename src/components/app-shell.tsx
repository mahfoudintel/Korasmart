        ...
        <nav className="fixed bottom-0 left-0 right-0 z-30 border-t border-white/10 bg-[#07100a]/70 px-3 py-2 backdrop-blur-lg lg:hidden">
          <div className="mx-auto grid max-w-md grid-cols-5 gap-1">
            {mobileItems.map((item) => {
              ...
              const Icon = item.icon;
              return (
                <Link key={item.href} href={item.href} className={cn("grid h-14 place-items-center rounded-2xl px-1 text-[10px] font-bold text-white/70", active && "bg-lime-300 text-black")}>
                  <Icon className="h-5 w-5" />
